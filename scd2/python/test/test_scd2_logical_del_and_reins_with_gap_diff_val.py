...
FILE_NAME = f"reports/{get_strategy_name().lower()}/scd2_test_logical_del_and_reins_with_gap_diff_val.md"
...
def test_step_1(ctx):
    ...
    render_init("Testing Reactivating a logically deleted record with a gap and a different value", FILE_NAME)
    render_data(
        "This test validates a REACTIVATE operation of a single entity. The reactivate is created by re-inserting the record in the raw table with a gap after the deleted version and with a different value.",
