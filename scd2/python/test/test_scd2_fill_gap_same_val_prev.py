...
FILE_NAME=f"reports/{get_strategy_name().lower()}/scd2_test_fill_gap_same_val_prev.md"
...
def test_step_1(ctx):
    ...
    render_init("Testing Filling a gap with the same value as the version before the gap", FILE_NAME)
    render_data("This test validates filling the gap in a single entity. The record added into the gap is having the same values as the version before the gap.", output_file_name=FILE_NAME)
...
def test_step_2(ctx):
    ...
    test_description = f"Fill the gap at {load_ts_2} by adding a record with the same values as the version before the gap."
