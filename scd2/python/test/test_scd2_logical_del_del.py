...
FILE_NAME="reports/scd2_test_logical_del_del.md"
...
def test_step_1():
    ...
    create_dim_table(conn, TRINO_CATALOG, TRINO_SCHEMA, DIM_TABLE_NAME, s3_warehouse_bucket=S3_WAREHOUSE_BUCKET, s3_warehouse_prefix=S3_WAREHOUSE_PREFIX, pk_col_with_type="id INT", cols_with_type=COLS_WITH_TYPE, partition_cols=["dp_valid_from"], sort_cols=[])
    render_init("Testing Logical Delete Operation and that it stays consistent", FILE_NAME)
...
def test_step_2():
    ...
    # run test
    scd2_merge_as_test(conn, test_step=2, ins_stmt=insert_sql_2, load_ts=load_ts_2, current_ts=current_ts_2, expected=expected, 
                        output_file_name=FILE_NAME, test_description=test_description)
    ...
    insert_sql_3 = f"""
        ...
        FROM (
            VALUES
                ...
                (3, 'Clara', 'Schmid', 'Basel', 'clara.schmid@example.com', 'INACTIVE', TIMESTAMP '{load_ts_3}')
        ) AS t (
            id,
            first_name,
            last_name,
