...
def test_step_1():
    ...
    render_data("This test validates a DELETE operation of a single record. The delete is created by a logical delete in the raw table, i.e., the record is set to INACTIVE. This test ensures that no further actions are taken with the same record in later partitions.", output_file_name=FILE_NAME)
