...
FILE_NAME=f"reports/{get_strategy_name().lower()}/scd2_test_logical_del_del_past.md"
...
def test_step_1(ctx):
    ...
    # run test
    scd2_merge_as_test(ctx, test_step=1, ins_stmt=insert_sql_1, load_ts=load_ts_1, current_ts=current_ts_1, expected=expected, 
...
def test_step_2(ctx):
    ...
    # run test
    scd2_merge_as_test(ctx, test_step=2, ins_stmt=insert_sql_2, load_ts=load_ts_3, current_ts=current_ts_3, expected=expected, 
                        output_file_name=FILE_NAME, test_description=test_description)
...
def test_step_3(ctx):
    ...
    insert_sql_3 = f"""
        ...
        FROM (
            VALUES
                ...
                (3, 'Clara', 'Schmid', 'Basel', 'clara.schmid@example.com', 'INACTIVE', TIMESTAMP '{load_ts_2}', TIMESTAMP '{load_ts_4}')
        ) AS t (
            id,
            first_name,
            last_name,
    ...
    # run test
    scd2_merge_as_test(ctx, test_step=3, ins_stmt=insert_sql_3, load_ts=load_ts_4, current_ts=current_ts_4, expected=expected, 
