...
from commons import TRINO_CATALOG, TRINO_SCHEMA, S3_WAREHOUSE_BUCKET, S3_WAREHOUSE_PREFIX, DIM_TABLE_NAME, RAW_TABLE_NAME, SCD2_VIEW_NAME, EXCLUDE_COLS, COLS_WITH_TYPE, scd2_merge_as_test, create_raw_table, init_trino_connection

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

FILE_NAME="reports/scd2_delta_test_logical_del.md"
...
def test_step_1():
    ...
    render_init("Testing Logical Delete Operation (Delta Mode for Source)", FILE_NAME)
...
def test_step_2():
    ...
    insert_sql_2 = f"""
        ...
        FROM (
            VALUES
                (3, 'Clara', 'Schmid', 'Basel', 'clara.schmid@example.com', 'INACTIVE', TIMESTAMP '{load_ts_2}')
        ) AS t (
            id,
            first_name,
            last_name,
    ...
    # run test
    scd2_merge_as_test(conn, test_step=2, ins_stmt=insert_sql_2, load_ts=load_ts_2, current_ts=current_ts_2, expected=expected, output_file_name=FILE_NAME, test_description=test_description, use_delta_mode_for_raw_table=True)
