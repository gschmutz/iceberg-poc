...
from commons import get_strategy_name, raw_table_fqn, scd2_table_fqn, S3_WAREHOUSE_BUCKET, S3_WAREHOUSE_PREFIX, RAW_TABLE_NAME, SCD2_VIEW_NAME, EXCLUDE_COLS, COLS_WITH_TYPE, create_dim_table_for_test, scd2_merge_as_test, create_raw_table

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

FILE_NAME=f"reports/{get_strategy_name().lower()}/scd2_test_upd_past_diff_val.md"
...
def test_step_3(ctx):
    ...
    # run test
    scd2_merge_as_test(ctx, test_step=3, ins_stmt=insert_sql, load_ts=load_ts_2, current_ts=current_ts_4, expected=expected, output_file_name=FILE_NAME, test_description=test_description, perform_merge_op=True)
