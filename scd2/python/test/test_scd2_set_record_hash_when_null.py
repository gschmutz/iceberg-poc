...
FILE_NAME = f"reports/{get_strategy_name().lower()}/scd2_test_set_record_hash_when_null.md"
...
def test_step_2(ctx):
    ...
    test_description = f"Clean (Nullify) all dp_record_hash values."

    ctx.conn.cursor().execute(f"UPDATE {scd2_table_fqn(ctx)} SET dp_record_hash = NULL")  # clean up SCD2 table for next test step

    scd2_sel_as_test(
        ctx,
        test_step=2,
        sel_stmt = f"SELECT * FROM {scd2_table_fqn(ctx)}",
        expected=None,
        output_file_name=FILE_NAME,
        test_description=test_description,
    )
...
def test_step_3(ctx):
    ...
    test_description = f"At {load_ts_2}, insert the new entity with `id=10` into the new partition of the raw table and then perform SCD2 merge with record hash update."
    ...
    scd2_merge_as_test(
        ...
        perform_record_hash_update=True,
    )
