...
FILE_NAME=f"reports/{get_strategy_name().lower()}/scd2_test_logical_del_and_reins_with_overlap_same_val.md"
...
def test_step_1(ctx):
    ...
    render_data("This test validates a REACTIVATE operation of a single entity. The reactivate is created by re-inserting the record with an overlap in the raw table.", output_file_name=FILE_NAME)
...
def test_step_3(ctx):
    ...
    test_description = "Reactivate record with `id=3` by inserting it again with an overlap to the deleted version and with exactly the same data as the deleted version and perform SCD2 merge."
