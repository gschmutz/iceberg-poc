...
DOWNLOAD_INITIAL_DATASET_FROM_S3 = get_param(
    "DOWNLOAD_INITIAL_DATASET_FROM_S3", "true"
).lower() in ("true", "1", "t")
...
def apply_daily_changes(fake: Faker, df: pd.DataFrame, next_person_id: int):
    ...
    updated["email"] = updated["email"].where(
        np.random.rand(len(updated)) >= 0.6, [fake.email() for _ in range(len(updated))]
    )
    ...
    updated["job_title"] = updated["job_title"].where(
        np.random.rand(len(updated)) >= 0.3, [fake.job() for _ in range(len(updated))]
    )
    ...
    new_rows = [
        generate_person_row(fake, next_person_id + i) for i in range(inserts_count)
    ]
    inserts = pd.DataFrame(new_rows)

    # Concatenate remaining, updated, logical_deleted, and new rows
    full_export = pd.concat(
        [remaining, updated, logical_deleted, inserts], ignore_index=True
    )
...
def run_raw_create_table(table_name: str):
    ...
    drop_table_stmt = (
        f"""DROP TABLE IF EXISTS {TRINO_CATALOG}.{TRINO_SCHEMA}.{table_name}"""
    )
...
def prepare_raw_data(use_hms: bool, generate_data: bool = True, initial_rows: int = 0):
    ...
    for d in range(NOF_DAYS):
        ...
        arrow_table = pa.Table.from_pandas(
            df, schema=arrow_schema, preserve_index=False
        )
