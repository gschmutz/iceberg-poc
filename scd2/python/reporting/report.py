...
def report_merge_op():
    merge_records = fetch_trino_rows(
        "SELECT * FROM benchmark_scd2_merge_report_v WHERE day_number IN (23)"
    )
    ...
    for ts in ts_keys:
        ...
        if arr.size == 0:
            logging.info(
                "Skipping percentile computation: no values left after filtering"
            )
...
def report_query_op():
    ...
    for base, stats in query_stats.items():
        ...
        if UPLOAD_TO_S3:
            ...
            logger.info(
                f"Uploaded query_summary_{base}.csv to s3://{S3_UPLOAD_BUCKET}/{s3_key}"
            )
